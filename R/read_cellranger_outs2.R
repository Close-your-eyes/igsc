#' Read and combine V(D)J contig and consensus annotations from Cell Ranger output
#'
#' Reads V(D)J output files generated by 10x Genomics Cell Ranger and returns a
#' single data frame containing contig-level annotations together with
#' clonotype consensus annotations. Optionally, nucleotide sequences from the
#' contig and consensus FASTA files can also be included.
#'
#' Compared with `read_cellranger_outs()`, this function preserves the original
#' contig-level CDR3 assignments and does not propagate consensus CDR3 sequences
#' to all contigs within the same clonotype. Consensus CDR3 information is
#' returned in the separate columns `cdr3_consensus` and
#' `cdr3_nt_consensus`.
#'
#' The following files are required for each sample:
#' \itemize{
#'   \item Always required:
#'   \itemize{
#'     \item \code{filtered_contig_annotations.csv}
#'     \item \code{consensus_annotations.csv}
#'   }
#'   \item Additionally required when \code{include_fasta = TRUE}:
#'   \itemize{
#'     \item \code{filtered_contig.fasta}
#'     \item \code{consensus.fasta}
#'   }
#' }
#'
#' @param vdj_outs_path A named character vector of paths to Cell Ranger V(D)J
#'   output directories (or directories containing the required files). Vector
#'   names are used as sample identifiers and added as the `sample` column in
#'   the returned data frame.
#' @param skip_incomplete Logical; if `TRUE`, samples missing one or more
#'   required files are skipped. If `FALSE` (default), the function stops with
#'   an error when required files are missing.
#' @param include_fasta Logical; if `TRUE` (default), contig and consensus
#'   nucleotide sequences are read from the corresponding FASTA files and added
#'   as `contig_seq` and `consensus_seq`. If `FALSE`, only annotation files are
#'   read.
#'
#' @return
#' A data frame containing one row per filtered contig. Depending on
#' `include_fasta`, the returned data frame includes:
#' \itemize{
#'   \item contig annotations from
#'     \code{filtered_contig_annotations.csv},
#'   \item consensus annotations from
#'     \code{consensus_annotations.csv},
#'   \item contig nucleotide sequences (`contig_seq`),
#'   \item consensus nucleotide sequences (`consensus_seq`), and
#'   \item consensus CDR3 amino acid (`cdr3_consensus`) and nucleotide
#'     (`cdr3_nt_consensus`) sequences.
#' }
#'
#' @seealso read_cellranger_outs()
#'
#' @examples
read_cellranger_outs2 <- function(vdj_outs_path,
  ...
  ff <- c("filtered_contig_annotations.csv", "consensus_annotations.csv")
  if (include_fasta) {
    ff <- c(ff, "consensus.fasta", "filtered_contig.fasta")
  }


  vdj_outs_path2 <- NULL
  for (i in vdj_outs_path) {
    files <- basename(list.files(i, recursive = T))
    if (anyDuplicated(files)) {
      stop("duplicate files of ", paste(ff, collapse = ", "), " found in vdj_outs_path.\nProvide one named char per sample folder. e.g.\npaths <- list.dirs(vdj_outs_path, recursive = F)\nnames(paths) <- basename(paths)")
    }

    if (!skip_incomplete) {
      if (any(!ff %in% files)) {
        stop(paste(ff[which(!ff %in% files)], collapse = ", "), " not found in ", i, ".")
      }
    } else {
      if (all(ff %in% files)) {
        vdj_outs_path2 <- c(vdj_outs_path2, i)
      }
    }
  }
  ...
  if (skip_incomplete) {
    ...
    if (length(vdj_outs_path2)) {
      message(length(vdj_outs_path2), " of ", length(vdj_outs_path), " samples are complete with ",  paste(ff, collapse = ", "), " found.")
      names(vdj_outs_path2) <- names(vdj_outs_path)[match(vdj_outs_path2, vdj_outs_path)]
      vdj_outs_path <- vdj_outs_path2
    } else {
      stop("no complete samples with ",  paste(ff, collapse = ", "), " found.")
    }
  }

  filt_cont_ann <- purrr::map_chr(vdj_outs_path, list.files, pattern = "filtered_contig_annotations\\.csv$", recursive = T, full.names = T)
  cons_ann <- purrr::map_chr(vdj_outs_path, list.files, pattern = "consensus_annotations\\.csv$", recursive = T, full.names = T)
  ...
  consensus_annotations <-
    ...
    dplyr::mutate(consensus_id = gsub("consensus_", "consensus", consensus_id)) # should not do anything here; consensus used to be without trailing underscore here, thats why

  if (include_fasta) {
    filt_cont_fast <- purrr::map_chr(vdj_outs_path, list.files, pattern = "filtered_contig\\.fasta$", recursive = T, full.names = T)
    cons_fast <- purrr::map_chr(vdj_outs_path, list.files, pattern = "consensus\\.fasta$", recursive = T, full.names = T)

    consensus_fasta <-
      purrr::map_dfr(cons_fast, ~tibble::enframe(read_fasta(.x)), .id = "sample") |>
      dplyr::rename("consensus_seq" = value, "consensus_id" = name)

    contig_fasta <-
      purrr::map_dfr(filt_cont_fast, ~tibble::enframe(read_fasta(.x)), .id = "sample") |>
      dplyr::rename("contig_seq" = value, "contig_id" = name)

    cl_long <- contig_annotations |>
      dplyr::left_join(contig_fasta, by = c("contig_id", "sample")) |>
      dplyr::left_join(consensus_fasta, by = c("consensus_id", "sample"))
  } else {
    cl_long <- contig_annotations
  }

  cl_long <- cl_long |>
    dplyr::mutate(consensus_id = gsub("consensus_", "consensus", consensus_id)) |>

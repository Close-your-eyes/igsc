#' Align paired reads to groups of reference sequences
#'
#' Find exact or approximate occurrences of paired-end reads within named groups
#' of DNA reference sequences. R1 reads are matched to the supplied references;
#' R2 reads are matched to their reverse complements. Consequently, all
#' sequences in `ref_seq_list` must use the plus-strand orientation.
#'
#' Reads are grouped by length and converted to [Biostrings::PDict()] objects.
#' Each dictionary is searched against every sequence in each reference group.
#' Matches retain the original paired-read index and the index of the matching
#' sequence within its reference group. Overall and per-group count summaries
#' are calculated after matching.
#'
#' @param r1 Character vector of R1 sequences. It must have the same length and
#'   ordering as `r2`; `NA` values are permitted.
#' @param r2 Character vector of R2 sequences paired positionally with `r1`.
#'   These reads are matched against reverse-complemented reference sequences.
#' @param r1_table Optional frequency table of non-missing R1 sequences, such as
#'   `table(r1, useNA = "no")`. When `NULL`, it is calculated internally. Its
#'   number of entries is used to normalize unique-read statistics.
#' @param r2_table Optional frequency table of non-missing R2 sequences, analogous
#'   to `r1_table`. When `NULL`, it is calculated internally.
#' @param ref_seq_list Named list whose elements are character vectors of
#'   plus-strand reference sequences. Outer list names identify reference groups;
#'   positions within each vector become `ref_seq_ind` values in match results.
#' @param mapply_fun Function, or its name, used to search combinations of
#'   reference groups and read lengths. Use [base::mapply()] for sequential
#'   execution or [parallel::mcmapply()] for multicore execution.
#' @param maxmis Non-negative numeric mismatch threshold. `0` requires exact
#'   matches; a value strictly between `0` and `1` allows
#'   `floor(read_length * maxmis)` mismatches; a value of at least `1` is treated
#'   as an absolute mismatch count. The count is capped below the read length.
#' @param ... Additional arguments passed to `mapply_fun`, such as `mc.cores`
#'   when using [parallel::mcmapply()].
#'
#' @return A named list with two components:
#'   \describe{
#'     \item{read_stats}{Overall read statistics in three data-frame layouts:
#'       `wide`, `long`, and `norm`, where `norm` places R1, R2, and paired
#'       values in separate columns. These report total and distinct read counts
#'       and frequencies.}
#'     \item{read_matches}{A list containing:
#'       \describe{
#'         \item{match_df_list}{Match data frames for `r1`, `r2`, and joined
#'           `r1_r2` results, grouped by reference-list name. Rows identify the
#'           original `read_ind`, matching `ref_seq_ind`, and read sequence.}
#'         \item{read_indices}{For each reference group, the paired,
#'           R1-only, and R2-only matching read indices.}
#'         \item{match_stats}{Per-reference match statistics in `wide`, `long`,
#'           and `norm` layouts.}
#'         \item{ref_seq_ind_matches}{Frequency tables of matching reference
#'           sequence indices for R1, R2, and complete pairs.}
#'       }
#'     }
#'   }
#' @export
...
#' @examples
#' \dontrun{
#' reads_paired <- igsc::read_paired_reads(fastq_path_r1 = r1_path,
#'                                         fastq_path_r2 = r2_path, min_len = 40,
#'                                         mc.cores = 8)
#' r1_table <- table(reads_paired[["reads"]][["r1"]][["seq"]], useNA = "no")
#' r2_table <- table(reads_paired[["reads"]][["r2"]][["seq"]], useNA = "no")
#' read_align_list <- igsc::align_reads(
#'   r1 =reads_paired[["reads"]][["r1"]][["seq"]],
#'   r2 = reads_paired[["reads"]][["r2"]][["seq"]],
#'   r1_table = r1_table,
#'   r2_table = r2_table,
#'   ref_seq_list = list(ref1 = c("ATCGTGCTG", "ACTTTTGGGCTTAAA"),
#'                       ref2 = c("AAAGGTTTCCCGTC", "TTGGCCAA", "GGCTA")),
#'   mc.cores = 2)
#'
#' # get ref_seq_list from downloaded genomes from NCBI
#' local_pathogen_ref <- list.files(file.path(wd, "pathogen_ref_sequences_origin"),
#'                                  full.names = T)
#' local_pathogen_ref <- local_pathogen_ref[which(!grepl("\\.xlsx|\\.fasta",
#'                                                       local_pathogen_ref))]
#' names(local_pathogen_ref) <- sapply(
#'   strsplit(basename(local_pathogen_ref), "__"),
#'   "[",
#'   2
#' )
#' cds_list_local <- purrr::map(
#'   local_pathogen_ref,
#'   get_cds_from_local_pathogen_ref,
#'   .progress = T
#' )
#'
...
#' feat_sequences <- igsc::get_seqs_from_feature_df(
#'   feature_df = feat_df,
...
#' r1 <- c("AACCGG", "TTGGCC", "AACCGG")
#' r2 <- c("CCGGTT", "GGCCAA", "CCGGTT")
#' references <- list(
#'   targets = c(
#'     gene_a = "TTTAACCGGAAA",
#'     gene_b = "CCCTTGGCCGGG"
...
#'   r1 = r1,
#'   r2 = r2,
#'   ref_seq_list = references,
#'   mapply_fun = mapply,
#'   maxmis = 0
#' )
#'
#' aligned$read_stats$wide
#' aligned$read_matches$match_df_list$r1$targets
#' }
...
match_reads <- function(reads,
  ...
  'read_ind_with_matches <- parallel::mclapply(stats::setNames(read_lengths_un, read_lengths_un), function(y) {
    print(y)
    if (maxmis < 0) {
      maxmis <- 0
      message("maxmis set to 0.")
  ...
  tt <- lapply(stats::setNames(names(ref_seq_list), names(ref_seq_list)), function(x) {

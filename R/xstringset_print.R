...
#' requires keep_match_gaps incompare_seq_df_wide
...
#' @param call_compare_seq_df_wide call igsc::compare_seq_df_wide to optionally
#' replace matches and/or mismatches
#' @param compare_seq_df_wide_args arguments to compare_seq_df_wide
#'
#' @returns xstringset
#' @export
#'
...
#' # print with call_compare_seq_df_wide anf GZMB as reference
#' xstringset_print(msaln, call_compare_seq_df_wide = T,
#'                  ref = "GZMB", print_pos_end = T)
#' # run compare_seq_df_wide  yourself
#' df <- igsc:::xstringset_to_df(msaln)
#' msalndf <- compare_seq_df_wide(df, ref = "GZMB",
#'                                seq_original = NULL,
#'                                change_nonref = T)
...
xstringset_print <- function(set,
                             ...
                             pos_by_ref = T,
                             col_out = T,
                             call_compare_seq_df_wide = F,
                             compare_seq_df_wide_args = list(change_nonref = T,
                                                             change_ref = F,
                                                             keep_match_gaps = T)) {
  ...
  if (call_compare_seq_df_wide) {
    ...
    if ("ref" %in% names(compare_seq_df_wide_args)) {
      ref <- compare_seq_df_wide_args[["ref"]]
      compare_seq_df_wide_args[["ref"]] <- NULL
    }
    type <- guess_type2(df[,1,drop=T]) # known from igsc:::xstringset_to_df
    df <- Gmisc::fastDoCall(compare_seq_df_wide,
                            args = c(list(df = df, ref = ref), compare_seq_df_wide_args))

    set <- df_to_xstringset(df, type = type, format = "wide")
    ref <- attr(df, "ref")
  }
  ...
  # in compare_seq_df_wide gaps may be converted to "." if they match the ref
  # in this case they cannot be counted by get_symbols and the sym_line is wrong
  # if such sequence becomes the ref here.
  # that actually requires to call compare_seq_df_wide appropriately
  message_printed <- F
  if (!is.null(ref) && pos_by_ref) {
    # when there is ref && pos_by_ref: make sym_line according to it
    if (seq_compared && !call_compare_seq_df_wide) {
      message_printed <- T
      message("'.' found in reference seq. when compare_seq_df_wide was called, keep_match_gaps = T may be required to allow counting of gaps in reference herein.")
    }
    sym_line <- paste(igsc:::get_symbols(s = strsplit(set[[ref]], "")[[1]], start.pos = 1), collapse = "")
  } else {
    # just a dummy seq w/o gaps
    sym_line <- paste(igsc:::get_symbols(s = rep("A", nchar(set[1])), start.pos = 1), collapse = "")
  }

  # make seq names equal in length
  names(set) <- igsc:::pad_strings(names(set))
  ...
  if (col_out) {
    ...
    set <- purrr::map(set, ~purrr::map_chr(.x, igsc:::col_letters, type = type, mc.cores = 1))
  }
  ...
  # add position at end
  if (print_pos_end) {
    if (seq_compared && !call_compare_seq_df_wide && !message_printed) {
      message("'.' found in reference seq. when compare_seq_df_wide was called, keep_match_gaps = T may be required to allow counting of gaps in reference herein.")
    }
  ...
  for (i in seq_along(sym_line)) {
    ...
    c(sym_line[i], pos) %<-% igsc:::.get_pos_line(name = pa_names[1], chunk = sym_line[i], pos = pos)
  }

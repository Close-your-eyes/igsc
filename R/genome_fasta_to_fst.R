...
genome_fasta_to_fst <- function(genome_file,
  ...
  for (y in seq_along(subfolder)) {
    subfolder[y] <- file.path(dirname(genome_file), y)
  ...
  for (x in seq_along(seq_bounds$seqname)) {
    ...
    if (!skip) {
      if (y == 1) {
        ...
        seq_bounds[which(seq_bounds$seqname == name), "lead_N"] <- nchar(chr) - nchar(gsub("^N+", "", chr))
        seq_bounds[which(seq_bounds$seqname == name), "trail_N"] <- nchar(chr) - nchar(gsub("N+$", "", chr))
      }

      if (grepl("fst$", subfolder[y])) {
        refseq <- data.frame(x = strsplit(x = refseq, split = "", fixed = T)[[1]])
        names(refseq) <- name
        fst::write_fst(refseq, path = out_path, compress = compression)
      } else {

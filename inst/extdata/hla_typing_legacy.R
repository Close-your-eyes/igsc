...
hla_typing <- function(hla_ref,
  ...
  if (!read_seq_col_name %in% names(reads)) {
    stop(read_seq_col_name, " not found in names of reads.")
  }
  if (!read_name_col_name %in% names(reads)) {
    stop(read_name_col_name, " not found in names of reads.")
  }
  if (!hla_seq_col_name %in% names(hla_ref)) {
    stop(hla_seq_col_name, " not found in names of hla_ref")
  }
  if (!p_group_col_name %in% names(hla_ref)) {
    stop(p_group_col_name, " not found in names of hla_ref")
  }
  if (!g_group_col_name %in% names(hla_ref)) {
    stop(g_group_col_name, " not found in names of hla_ref")
  }
  if (!hla_allele_col_name %in% names(hla_ref)) {
    stop(hla_allele_col_name, " not found in names of hla_ref")
  }

  if (anyDuplicated(reads[,read_name_col_name,drop=T])) {
    message("Duplicated ", read_name_col_name, " found. Will make them unique.")
    reads[,read_name_col_name] <- make.unique(reads[,read_name_col_name,drop=T])
  }

  if (length(invalid <- which(grepl("[^ACTGU]", reads[,read_seq_col_name,drop=T]))) > 0) {
  ...
  genes <- table(sapply(strsplit(sapply(strsplit(hla_ref$allele, "-"), "[", 2), "\\*"), "[", 1))
  if (length(genes) > 1) {
    message("More than one HLA gene detected. Did you forget to filter for one reference allele only.")
    print(genes)
  }

  hla_ref <- hla_ref[,colSums(is.na(hla_ref)) < nrow(hla_ref)]
  hla_ref <- dplyr::distinct(hla_ref, !!rlang::sym(hla_seq_col_name), .keep_all = TRUE)


  if (make_reads_distinct) {
    n_before <- nrow(reads)
    reads <- dplyr::distinct(reads, !!rlang::sym(read_seq_col_name), .keep_all = T)
    n_after <- nrow(reads)
    if (n_after < n_before) {
  ...
  #
  # hla_ref_top <- dplyr::filter(hla_ref, !!rlang::sym(hla_allele_col_name) %in% unique(top_alleles))
  #
  # hla_ref_top_complete <- hla_ref |>
  #   dplyr::filter(!!rlang::sym(p_group_col_name) %in% unique(hla_ref_top[[p_group_col_name]])) |>
...
run_read_matching_and_report_results <- function(hla_ref,
  ...
  pair_res_df <- pair_res_df |>
    ...
    dplyr::rename("p_group1" = p_group_col_name, "g_group1" = g_group_col_name)|>
    ...
    dplyr::rename("p_group2" = p_group_col_name, "g_group2" = g_group_col_name)|>
...
single_matching <- function(reads,
  ...
  reads <- stats::setNames(reads[,read_seq_col_name,drop=T], reads[,read_name_col_name,drop=T])
  single_res <- do.call(rbind, lapply_fun(split(reads, ceiling(seq_along(reads)/1e3)), function(rrr) {
    ...
    hit_matrix <- Biostrings::vwhichPDict(subject = Biostrings::DNAStringSet(hla_ref[,hla_seq_col_name,drop=T]),
    ...
    colnames(hit_matrix) <- hla_ref[,hla_allele_col_name,drop=T]

...
Even though CellRanger annotates VDJ chain, we may repeat that ourselves as well. Also this brings the IMGT nomenclature into our data frame. For that we have to prepare a respective data frame from the data available on IMGTs website.
...
V_matches <- sapply(unique(clonotype.data.long.format$V), function(x) {
  ...
  min_val <- min(dists)
  ref_al[which(dists == min_val)]
})
J_matches <- sapply(unique(clonotype.data.long.format$J), function(x) {
  ref_al <- imgt_ref$Allele[which(grepl(stringr::str_extract(x, "^[:alpha:]{1,}[:digit:]{1,}"), imgt_ref$Allele))]
  dists <- adist(x, ref_al, ignore.case = T)
  min_val <- min(dists)
  ref_al[which(dists == min_val)]
})
clonotype.data.long.format$V.allele <- V_matches[clonotype.data.long.format$V]
clonotype.data.long.format$J.allele <- J_matches[clonotype.data.long.format$J]

# then resolve allele-groups for which there are multiple alleles in IMGT
V_unique <-
  clonotype.data.long.format %>%
  dplyr::distinct(cell.ranger.consensus.seq, V.allele, clonotype_name)

V_unique$V.allele <- unlist(pbapply::pblapply(1:nrow(V_unique), function(x) {
  als <- V_unique$V.allele[x]
  if (length(unlist(als)) == 1) {
    return(unname(unlist(als)))
  } else {
    seq <- V_unique$cell.ranger.consensus.seq[x]
    refs <- imgt_ref[which(imgt_ref$Allele %in% unlist(als)),"seq.nt"]
    max.ind <- which.max(Biostrings::pairwiseAlignment(subject = Biostrings::DNAString(seq), pattern = Biostrings::DNAStringSet(refs), type = "local", scoreOnly = T))
    return(imgt_ref[which(imgt_ref$Allele %in% unlist(als)),"Allele"][max.ind])
  }
}))

J_unique <-
  clonotype.data.long.format %>%
  dplyr::distinct(cell.ranger.consensus.seq, J.allele, clonotype_name)

J_unique$J.allele <- unlist(pbapply::pblapply(1:nrow(J_unique), function(x) {
  als <- J_unique$J.allele[x]
  if (length(unlist(als)) == 1) {
    return(unname(unlist(als)))
  } else {
    seq <- J_unique$cell.ranger.consensus.seq[x]
    refs <- imgt_ref[which(imgt_ref$Allele %in% unlist(als)),"seq.nt"]
    max.ind <- which.max(Biostrings::pairwiseAlignment(subject = Biostrings::DNAString(seq), pattern = Biostrings::DNAStringSet(refs), type = "local", scoreOnly = T))
    return(imgt_ref[which(imgt_ref$Allele %in% unlist(als)),"Allele"][max.ind])
  }
...
clonotype.data.long.format <-
  clonotype.data.long.format %>%
  dplyr::select(-c(J.allele, V.allele)) %>%
  dplyr::left_join(V_unique, by = c("cell.ranger.consensus.seq", "clonotype_name")) %>%
  dplyr::left_join(J_unique, by = c("cell.ranger.consensus.seq", "clonotype_name"))
...
```
```{r eval=T, include=F}
clonotype.data.wide.format <- readRDS("clonotype_data.rds")[["wide"]]
```
```{r eval=T}
# test if every name is unique to cell.ranger.clonotype.id
check <-
  clonotype.data.wide.format %>%

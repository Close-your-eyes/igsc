...
#pbapply::pblapply for process bar and alternatively to mclapply
V_unique$V_imgt <- unlist(parallel::mclapply(1:nrow(V_unique), function(x) {
  als <- unique(unlist(V_unique$V_imgt[x]))
  if (length(als) == 1) {
    return(unname(als))
  } else {
...
J_unique$J_imgt <- unlist(parallel::mclapply(1:nrow(J_unique), function(x) {
  als <- unique(unlist(J_unique$J_imgt[x]))
  if (length(als) == 1) {
    return(unname(als))
  } else {
    ...
    refs <- imgt_ref[which(imgt_ref$Allele %in% als),]
    max.ind <- which.max(Biostrings::pairwiseAlignment(subject = Biostrings::DNAString(seq), pattern = Biostrings::DNAStringSet(refs[,"seq.nt"]), type = "local", scoreOnly = T))
    max.ind <- sample(1:length(max.ind), 1)
    return(refs[max.ind,"Allele"])
  }
}, mc.cores = parallel::detectCores()))
